...
char** placeToPlay(piece chess[TAILLETAB][TAILLETAB], char pos[3],WINDOW *boxv[TAILLETAB][TAILLETAB],int height,int width,coord basePlacement,int affichage,int verrif){
    ...
    if (isAlpha(pion.type)){
        ...
        switch(pion.type){
            ...
            if (pion.type!='V'+32*isLowerCase(pion.type)){
                break;
            }
            ...
            for(int i=0;i<pion.taille;i++){
                if (pion.type!='V'+32*isLowerCase(pion.type)){
                    ...
                    if (pos[1]-49+pion.move[vaillant][1]-49>=0&&pos[1]-49+pion.move[vaillant][1]-49<TAILLETAB&&pos[0]-65+pion.move[vaillant][0]-65>=0&&pos[0]-65+pion.move[vaillant][0]-65<TAILLETAB){
                    
                        if((pos[1]-49+pion.move[vaillant][1]-49>=0&&pos[1]-49+pion.move[vaillant][1]-49<TAILLETAB)&&(pos[0]-65+pion.move[vaillant][0]-65>=0&&pos[0]-65+pion.move[vaillant][0]-65<TAILLETAB)){
                            if(chess[pos[1]-49+pion.move[vaillant][1]-49][pos[0]-65+pion.move[vaillant][0]-65].type==0){
                                if(affichage){
                                    wbkgd(boxv[pos[1]-49+pion.move[vaillant][1]-49][pos[0]-65+pion.move[vaillant][0]-65], COLOR_PAIR(3));
                                    wrefresh(boxv[pos[1]-49+pion.move[vaillant][1]-49][pos[0]-65+pion.move[vaillant][0]-65]);
                                }
                                strcpy(tab[cursor],pion.move[vaillant]);
                                cursor++;
                            }
                            else if((isUpperCase(chess[pos[1]-49+pion.move[vaillant][1]-49][pos[0]-65+pion.move[vaillant][0]-65].type) != isUpperCase(chess[pos[1]-49][pos[0]-65].type))||verrif){
                                if(affichage){
                                    wbkgd(boxv[pos[1]-49+pion.move[vaillant][1]-49][pos[0]-65+pion.move[vaillant][0]-65], COLOR_PAIR(4));
                                    wrefresh(boxv[pos[1]-49+pion.move[vaillant][1]-49][pos[0]-65+pion.move[vaillant][0]-65]);
                                }
                                strcpy(tab[cursor],pion.move[vaillant]);
                                cursor++;
