...
int *checkMate(piece chess[TAILLETAB][TAILLETAB], int verifCheck[TAILLETAB][TAILLETAB][2], char strTemp[3], WINDOW *boxv[TAILLETAB][TAILLETAB], int hauteur, int largeur, coord basePlacement, piece piecesEchequier[TAILLEPPIECEECHECS]) {
        ...
        for (int y=0;y<TAILLETAB;y++){
            for (int x=0;x<TAILLETAB;x++){
                ...
                if (chess[x][y].type == 'K'+32*z){
                    ...
                    checkMoves = placeToPlay(chess,strTemp,boxv,hauteur,largeur,basePlacement,affichage,verrif);


                    while(strcmp(checkMoves[counter],"11")!=0){


                        pos[0] = y + checkMoves[counter][1] - '1' ;
                        pos[1] = x + checkMoves[counter][0] - 'A' ;
                        pos[2] = '\0';
                        ...
                        if(verifCheck[pos[0]][pos[1]][0]==verifCheck[pos[0]][pos[1]][1]&&count){
                            ...
                            tempChessTable[pos[0]][pos[1]] = piecesEchequier[0];

                            if(checkVerrif(verifCheck,tempChessTable,boxv,hauteur,largeur,basePlacement)[z]){
                                returnCheck[z]=1;
                            }
                        }
                        counter ++;
 ...
 /*for (int j=0;j<2;j++){
        for (int i=0;i<TAILLETAB;i++){
            for (int e=0;e<TAILLETAB;e++){
                if (verifCheck[i][e][j]!=0){
                    printw("%d",verifCheck[i][e][j]);
                }
                else{
                    printw(" ");
                }
    ...
    /*for (int i = 0; i < TAILLETAB; i++) {
        for (int e = 0; e < TAILLETAB; e++) {
            if (chess[i][e].type == 'K' + 32 * isLowerCase(chess[i][e].type)) {
                if (verifCheck[i][e][isUpperCase(chess[i][e].type)] != 0) {
                    printw("ok");
                    return 0; // King is under check, not a checkmate
                }
                count = 0;
                char** allMoves = placeToPlay(chess, strTemp, boxv, hauteur, largeur, basePlacement, affichage, verrif);
                while (strcmp(allMoves[count], "11") != 0) {
                    resetTabFunc(tempChessTable);
                    for (int row = 0; row < TAILLETAB; row++) {
                        for (int col = 0; col < TAILLETAB; col++) {
                            if (isAlpha(chess[row][col].type)) {
                                tempChessTable[row][col] = chess[row][col];
                            }
                        }
                    }
                    tempChessTable[strTemp[1] - '1'][strTemp[0] - 'A'] = piecesEchequier[0];
                    int* verrifTab = checkVerrif(verifCheck, tempChessTable, boxv, hauteur, largeur, basePlacement);
                    if (verrifTab[isLowerCase(chess[strTemp[1] - '1'][strTemp[0] - 'A'].type)] != 0) {
                        printw("ok 1");
                        return 0; // King is still under check, not a checkmate
                    }
                    count++;
                }
            }
...
int *game(piece chessTable[TAILLETAB][TAILLETAB],piece piecesEchequier[TAILLEPPIECEECHECS],int tour,int verifCheck[TAILLETAB][TAILLETAB][2]){
                    ...
                    if (tour*32+65<=chessTable[placement.y][placement.x].type && tour*32+90 >= chessTable[placement.y][placement.x].type&&!selection){
                        ...
                        if (selection){
                            ...
                            if(!verrifTab[!tour]){
                                ...
                                int *mateOrNot = checkMate(tempChessTable,verifCheck,str,boxv,hauteur,largeur,basePlacement,piecesEchequier);
