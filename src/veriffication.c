...
//génère un tableau qui contient les mouvements possibles de la pièce sélectionnée
char** placeToPlay(piece chess[TAILLETAB][TAILLETAB], char pos[3],WINDOW *boxv[TAILLETAB][TAILLETAB],int height,int width,coord basePlacement,int affichage,int verrif){
    ...
    if (isAlpha(pion.type)){
        ...
        switch(pion.type){//en fonction de la pièce, on ajoute les mouvements dans le tableau
            case 'b':
            case 'B':
            case 'r':
            case 'R': 
            ...
            for (int e=0;e<4;e++){
                for(int i=0;i<(pion.taille-8)/4;i++){
                    if (pos[1]-49+pion.move[i+e*(pion.taille-8)/4][1]-49>=0&&pos[1]-49+pion.move[i+e*(pion.taille-8)/4][1]-49<TAILLETAB&&pos[0]-65+pion.move[i+e*(pion.taille-8)/4][0]-65>=0&&pos[0]-65+pion.move[i+e*(pion.taille-8)/4][0]-65<TAILLETAB){
                        

                        if(chess[pos[1]-49+pion.move[i+e*(pion.taille-8)/4][1]-49][pos[0]-65+pion.move[i+e*(pion.taille-8)/4][0]-65].type==0){
                            if(affichage){
                                wbkgd(boxv[pos[1]-49+pion.move[i+e*(pion.taille-8)/4][1]-49][pos[0]-65+pion.move[i+e*(pion.taille-8)/4][0]-65], COLOR_PAIR(3));
                                wrefresh(boxv[pos[1]-49+pion.move[i+e*(pion.taille-8)/4][1]-49][pos[0]-65+pion.move[i+e*(pion.taille-8)/4][0]-65]);
                            }    
                            strcpy(tab[cursor],pion.move[i+e*(pion.taille-8)/4]);
                            cursor++;
                                
                        }
                        else if((isUpperCase(chess[pos[1]-49+pion.move[i+e*(pion.taille-8)/4][1]-49][pos[0]-65+pion.move[i+e*(pion.taille-8)/4][0]-65].type) != isUpperCase(chess[pos[1]-49][pos[0]-65].type))||verrif){
                            if(affichage){
                                wbkgd(boxv[pos[1]-49+pion.move[i+e*(pion.taille-8)/4][1]-49][pos[0]-65+pion.move[i+e*(pion.taille-8)/4][0]-65], COLOR_PAIR(4));
                                wrefresh(boxv[pos[1]-49+pion.move[i+e*(pion.taille-8)/4][1]-49][pos[0]-65+pion.move[i+e*(pion.taille-8)/4][0]-65]);
                            }    
                            strcpy(tab[cursor],pion.move[i+e*(pion.taille-8)/4]);
                            cursor++;
            ...
            for(int i=pion.taille-8;i<pion.taille;i++){
            ...
            for (int e=0;e<8;e++){
                for(int i=0;i<(pion.taille-8)/8;i++){
                    if (pos[1]-49+pion.move[i+e*(pion.taille-8)/8][1]-49>=0&&pos[1]-49+pion.move[i+e*(pion.taille-8)/8][1]-49<TAILLETAB&&pos[0]-65+pion.move[i+e*(pion.taille-8)/8][0]-65>=0&&pos[0]-65+pion.move[i+e*(pion.taille-8)/8][0]-65<TAILLETAB){
                        
                        
                        if(chess[pos[1]-49+pion.move[i+e*(pion.taille-8)/8][1]-49][pos[0]-65+pion.move[i+e*(pion.taille-8)/8][0]-65].type==0){
                            if(affichage){
                                wbkgd(boxv[pos[1]-49+pion.move[i+e*(pion.taille-8)/8][1]-49][pos[0]-65+pion.move[i+e*(pion.taille-8)/8][0]-65], COLOR_PAIR(3));
                                wrefresh(boxv[pos[1]-49+pion.move[i+e*(pion.taille-8)/8][1]-49][pos[0]-65+pion.move[i+e*(pion.taille-8)/8][0]-65]);
                            }    
                            strcpy(tab[cursor],pion.move[i+e*(pion.taille-8)/8]);
                            cursor++;
                            
                        }
                        else if((isUpperCase(chess[pos[1]-49+pion.move[i+e*(pion.taille-8)/8][1]-49][pos[0]-65+pion.move[i+e*(pion.taille-8)/8][0]-65].type) != isUpperCase(chess[pos[1]-49][pos[0]-65].type))||verrif){
                            if(affichage){
                                wbkgd(boxv[pos[1]-49+pion.move[i+e*(pion.taille-8)/8][1]-49][pos[0]-65+pion.move[i+e*(pion.taille-8)/8][0]-65], COLOR_PAIR(4));
                                wrefresh(boxv[pos[1]-49+pion.move[i+e*(pion.taille-8)/8][1]-49][pos[0]-65+pion.move[i+e*(pion.taille-8)/8][0]-65]);
                            }    
                            strcpy(tab[cursor],pion.move[i+e*(pion.taille-8)/8]);
                            cursor++;
            ...
            if (pion.hasMove==0){//pour les pions on doit vérifier si ils ont déjà bouger ou pas
...
//on vérifie si un echec est présent afin de verifier si il y a un mat ou non
...
//on vérifie si il y a un mat ou non en comparant les tableaux d'aiiaques
