            ...
            for(int e=0;e<TAILLE_MOVE-1;e++){
                if(strcmp(listWords[i],"11")){
                    ...
                    if((isLowerCase(pion.type)*10+1==selectionjeux[1]-49)&&pion.type=='P'+32*isLowerCase(pion.type)){
                        chess[selectionjeux[1]-49][selectionjeux[0]-65] = piecesEchequier[isLowerCase(pion.type)];
                    }
    ...
    for (int i=0;i<TAILLETAB;i++){
        ...
        for (int e=0;e<TAILLETAB;e++){
            ...
            if (isAlpha(chess[i][e].type)){
                ...
                //char **allMooves = removeDuplicates(placeToPlay(chess,pos,boxv,hauteur,largeur,basePlacement,affichage,verrif));

                /*printw("%c ",chess[i][e].type);
                printw("%c %c ||", pos[0],pos[1]);

                char** allMooves = (placeToPlay(chess,pos,boxv,hauteur,largeur,basePlacement,affichage,verrif));
                int count=0;
                while(allMooves[count]!=NULL){
                    printw("%c %c |", allMooves[count][0],allMooves[count][1]);
                    printw("%d %d", pos[0]-2*'A'+allMooves[count][0],pos[1]-2*'1'+allMooves[count][1]);
                    count++;
                }
                                       printw("%c ",chess[i][e].type);
                        printw("%c %c ||", pos[0],pos[1]);
                ...
                if (placement.x>=0&&placement.x<TAILLETAB && placement.y>=0&&placement.y<TAILLETAB){
                    ...
                    if (tour*32+65<=chessTable[placement.y][placement.x].type && tour*32+90 >= chessTable[placement.y][placement.x].type&&!selection){
                        ...
                        if (selection){
                            ...
                            printw("%d",checkVerrif(verifCheck,chessTable ,boxv, hauteur, largeur, basePlacement));                       
